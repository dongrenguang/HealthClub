...
public class WaiterDaoImpl implements WaiterDao{
	...
	public boolean login(String username, String password) {
		...
		try {
			stmt=con.prepareStatement("select * from waiters where name=? and password=?");	
	...
	public Users getUser(int id){
		...
		try {
			...
			if(result.next()){
				Users user=new Users(id, result.getString(2), result.getString(3), result.getString(4),
						result.getString(5), result.getDate(6),result.getString(7), result.getString(8), 
						result.getString(9), result.getDate(10), result.getInt(11));
			    return user;
			}else {
				return null;
			}
	...
	public ArrayList<Session1> getOrderedAct(int uid){
		...
		try {
			stmt=con.prepareStatement("select * from session s where exists(select * from joinin j where j.uid=? and j.sid=s.id and j.type=?)");
			stmt.setInt(1,uid);
			stmt.setInt(2,0);			
			result=stmt.executeQuery();		
	...
	public boolean Join(int uid,int sid){
		...
		PreparedStatement stmt=null;
		try {
		stmt=con.prepareStatement("update joinin set type=? where uid=? and sid=?");
		stmt.setInt(1,1);
		stmt.setInt(2,uid);
		stmt.setInt(3,sid);
	...
	public boolean DeleteAct(int uid,int sid){
		...
		try {
			stmt=con.prepareStatement("delete from joinin where uid=? and sid=? and type=?");
			stmt.setInt(1,uid);
			stmt.setInt(2,sid);
			stmt.setInt(3,1);
			result=stmt.execute();
	...
	public boolean saveSessions(ArrayList<Session1> session1s){
		...
		try {
			for(int i=0;i<session1s.size();i++){
				...
				stmt=con.prepareStatement("insert into session(aid,time,location,coach,theme)"
						+ "values(?,?,?,?,?)");
	...
	public boolean saveActivity(Activity activity){
		...
		try {
			stmt=con.prepareStatement("update activity set theme=?,picture=? where id=?");
	...
	public boolean modifySession(ArrayList<Session1> sessionList){
		...
		try {
			for(int i=0;i<sessionList.size();i++){
				...
				stmt=con.prepareStatement("update session set time=?,location=?,coach=?,theme=? where id=?");
